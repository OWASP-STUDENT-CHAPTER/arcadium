...
router.post('/buy', isAuthenticated, async (req, res) => {
  ...
  req.user.game.property += property.price;
  req.user.game.points =
    req.user.game.money * 0.3 + 0.7 * req.user.game.property;
  room.ownershipMap[property._id] = req.user._id;
  console.log(
    'Points:' + req.user.game.points,
    req.user.game.money,
    req.user.game.property
  );
  await room.save();
  await req.user.save();
